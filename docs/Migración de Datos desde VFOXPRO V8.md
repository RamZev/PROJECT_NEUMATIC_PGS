...
Revisar si los modelos siguientes no tienen estos cambios, proceda a aplicarlos:  

**Localidad**

nombre_localidad -> max_length=50

**Empresa**

cuit -> BigIntegerField()

**Sucursal**

telefono_sucursal -> max_length=20
...
numero_comprobante -> BigIntegerField()
...
documento_asociado -> max_length=12

numero_comprobante -> BigIntegerField()

numero_comprobante_venta -> BigIntegerField()

**CajaDetalle**

idventas-> BigIntegerField()

idcompras-> BigIntegerField()

idbancos -> BigIntegerField()

**Chequerecibo**

codigo_banco-> BigIntegerField()
codigo_postal-> BigIntegerField()
cuenta_cheque_recibo-> BigIntegerField()
cuit_cheque_recibo-> BigIntegerField()
numero_cheque_recibo on chequerecibo
sucursal on chequerecibo
cuotas on tarjetarecibo
cupon on tarjetarecibo
lote on tarjetarecibo

## 5. Ejecutar la secuencia de los archivos generales de migración:

COPY (
    SELECT json_agg(row_to_json(t)) 
    FROM comprobante_venta AS t
) TO 'd:/neumatic_bak/comprobante_venta.json';

COPY (
    SELECT json_agg(row_to_json(t)) 
    FROM comprobante_compra AS t
) TO 'd:/neumatic_bak/comprobante_compra.json';
