# neumatic/data_load/padron_entrerios_migra.py
import os
import sys
import csv
...
from apps.maestros.models.padron_models import PadronEntreRiosIIBB

# Tamaño del lote
LOTE = 10000

def reset_tabla():
    PadronEntreRiosIIBB.objects.all().delete()
    print("Tabla padron_entre_rios limpiada.")
    try:
        with connection.cursor() as cursor:
            cursor.execute("SELECT setval(pg_get_serial_sequence('padron_entre_rios', 'id_padron'), 1, false);")
...
def cargar_datos():
    ...
    archivo = os.path.join(BASE_DIR, 'data_load', 'padron', 'PadronRetPer202609 ENTRE RIOS.csv')
    ...
    with open(archivo, 'r', encoding='latin-1') as f:
        ...
        for row in reader:
            ...
            if len(lote) >= LOTE:
                PadronEntreRiosIIBB.objects.bulk_create(lote)
    ...
    print("="*60)
    print("Migración de Padrón Entre Ríos completada exitosamente.")
