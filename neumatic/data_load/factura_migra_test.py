...
total_regs = 200000
BATCH_SIZE = 5000  # Probar con lotes pequeños
...
def procesar_registros(limite=total_regs):
    print("=" * 60)
    print(f"PROCESANDO PRIMEROS {limite} REGISTROS EN LOTES DE {BATCH_SIZE}")
    print("=" * 60)
    ...
    for idx, record in enumerate(table):
        ...
        try:
            ...
            batch.append(factura)
            
            # Cuando el lote está lleno, guardar con bulk_create
            if len(batch) >= BATCH_SIZE:
                try:
                    Factura.objects.bulk_create(batch)
                    facturas_creadas += len(batch)
                    batch_num += 1
                    print(f"✅ Lote {batch_num}: {len(batch)} facturas guardadas - Total OK: {facturas_creadas}")
                    batch = []
                except Exception as e:
                    error_log.info(f"ERROR EN LOTE: {str(e)[:200]}")
                    # Si falla bulk_create, guardar individualmente
                    for fact in batch:
                        try:
                            fact.save()
                            facturas_creadas += 1
                        except Exception as e2:
                            error_log.info(f"Individual falló ID:{fact.id_factura}: {str(e2)[:100]}")
                            errores += 1
                    batch = []
            
            if (idx + 1) % 500 == 0:
                print(f"Procesados {idx+1}/{limite} registros - OK: {facturas_creadas} - Error: {errores}")
    ...
    if batch:
        try:
            Factura.objects.bulk_create(batch)
            facturas_creadas += len(batch)
            print(f"✅ Lote final: {len(batch)} facturas guardadas")
        except Exception as e:
            error_log.info(f"ERROR EN LOTE FINAL: {str(e)[:200]}")
            for fact in batch:
                try:
                    fact.save()
                    facturas_creadas += 1
