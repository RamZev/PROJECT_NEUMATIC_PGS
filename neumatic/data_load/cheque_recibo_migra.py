        ...
        with transaction.atomic():
            ...
            with connection.cursor() as cursor:
                if 'sqlite' in engine:
                    ...
                    cursor.execute("SELECT setval(pg_get_serial_sequence('cheque_recibo', 'id_cheque_recibo'), 1, false);")
                    print("<info>Secuencia de ID reseteada (PostgreSQL).</info>")
                elif 'mssql' in engine or 'sql_server' in engine:
                    cursor.execute("DBCC CHECKIDENT ('cheque_recibo', RESEED, 0);")
                    print("<info>Secuencia de ID reseteada (SQL Server).</info>")
                elif 'mysql' in engine:
                    cursor.execute("ALTER TABLE cheque_recibo AUTO_INCREMENT = 1;")
