    ...
    try:
        ...
        with connection.cursor() as cursor:
            if 'sqlite' in engine:
                ...
                cursor.execute("SELECT setval(pg_get_serial_sequence('cliente', 'id_cliente'), 1, false);")
                print("Secuencia de ID reseteada (PostgreSQL).")
            elif 'mssql' in engine or 'sql_server' in engine:
                cursor.execute("DBCC CHECKIDENT ('cliente', RESEED, 0);")
                print("Secuencia de ID reseteada (SQL Server).")
            elif 'mysql' in engine:
                cursor.execute("ALTER TABLE cliente AUTO_INCREMENT = 1;")
...
def precargar_relaciones():
    ...
    # Usar values_list para evitar el bug del driver mssql
    relaciones = {
        ...
        'vendedores': {v[0]: Vendedor(id_vendedor=v[0]) for v in Vendedor.objects.values_list('id_vendedor')},
        'actividades': {a[0]: Actividad(id_actividad=a[0]) for a in Actividad.objects.values_list('id_actividad')},
        'sucursales': {s[0]: Sucursal(id_sucursal=s[0]) for s in Sucursal.objects.values_list('id_sucursal')},
        'percepciones': {p[0]: TipoPercepcionIb(id_tipo_percepcion_ib=p[0]) for p in TipoPercepcionIb.objects.values_list('id_tipo_percepcion_ib')},
        'localidades': {loc.codigo_postal: loc for loc in Localidad.objects.select_related('id_provincia').all()}
    }
    
    print(f"📦 Vendedores precargados: {len(relaciones['vendedores'])}")
    print(f"📦 Actividades precargadas: {len(relaciones['actividades'])}")
    print(f"📦 Sucursales precargadas: {len(relaciones['sucursales'])}")
    print(f"📦 Percepciones precargadas: {len(relaciones['percepciones'])}")
    print(f"📦 Localidades precargadas: {len(relaciones['localidades'])}")
