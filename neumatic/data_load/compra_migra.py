    ...
    try:
        with connection.cursor() as cursor:
            if 'sqlite' in engine:
                ...
                cursor.execute("SELECT setval(pg_get_serial_sequence('compra', 'id_compra'), 1, false);")
                ...
                cursor.execute("ALTER TABLE compra AUTO_INCREMENT = 1;")
                print("Secuencia de ID reseteada (MySQL).")
            else:
                print(f"Motor {engine} no requiere reset manual de secuencia.")
        
        logger.info("Tabla Compra limpiada y secuencia reseteada.")
...
def cargar_datos_compra():
    ...
    try:
        ...
        batch_size = 3000  # Cambiado de 1000 a 3000
        compras_batch = []
        ...
        for idx, record in enumerate(table, 1):
            ...
            try:
                ...
                # Crear instancia usando el ID original del DBF
                compra = Compra(
                    id_compra=codigo_origen,  # ← Usar el ID del DBF, NO contador
