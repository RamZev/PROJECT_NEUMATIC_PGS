...
CREATE VIEW VLComisionVendedor AS 
	SELECT 
		...
		f.id_cliente_id,
		c.nombre_cliente,
		CAST('' AS character varying(1)) AS reventa,
		0 AS id_producto_id,
		CAST('' AS character varying(15)) AS medida,
		0 AS id_marca_id,
		CAST('' AS character varying(50)) AS nombre_producto_marca,
		0 AS id_familia_id,
		CAST('' AS character varying(50)) AS nombre_producto_familia,
		CAST(0.0 AS numeric) AS cantidad,
		CAST(0.0 AS numeric) AS precio,
		CAST(0.0 AS numeric) AS costo,
		CAST(0.0 AS numeric) AS descuento,
		ROUND(f.total / (((SELECT alicuota_iva FROM codigo_alicuota WHERE codigo_alicuota = '0005')/100.0)+1), 2) AS gravado,
		f.total,
		CAST('false' AS boolean) AS no_estadist,
		CASE
