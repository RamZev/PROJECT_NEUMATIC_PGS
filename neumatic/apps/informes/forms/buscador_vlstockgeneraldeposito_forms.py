# neumatic\apps\informes\forms\buscador_vlstockgeneraldeposito_forms.py
...
class BuscadorStockGeneralDepositoForm(InformesGenericForm):
	...
	depositos = forms.ModelMultipleChoiceField(
		...
		required=True,
		label="Seleccione Depósitos a detallar"
	)
	...
	def clean(self):
		...
		depositos = cleaned_data.get('depositos') or []
		...
		if len(depositos) > 8:
			self.add_error("depositos", f"Debe seleccionar un máximo de 8 depósitos. Ha seleccionado {len(depositos)}.")
