# apps\informes\views\vlreposicionstock_list_views.py
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reposición de Stock"
	
	#-- Modelo.
	model = VLReposicionStock
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorReposicionStockForm
...
class VLReposicionStockInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		queryset = VLReposicionStock.objects.obtener_datos(
	...
	def obtener_contexto_reporte(self, queryset, cleaned_data):
		...
		sucursales_info = _sucursal_info(Sucursal.objects.filter(estatus_sucursal=True))
...
def vlreposicionstock_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def vlreposicionstock_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = VLReposicionStockInformeView()
...
def vlreposicionstock_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	info = [
		{
			...
			'nombre': sucursal.nombre_sucursal,					#-- Nombre de la sucursal.
