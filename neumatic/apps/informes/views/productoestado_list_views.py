# neumatic\apps\informes\views\productoestado_list_views.py
...
from ..forms.buscador_productoestado_forms import BuscadorProductoEstadoForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Producto Estado"
	
	#-- Modelo.
	model = ProductoEstado
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorProductoEstadoForm
...
class ProductoEstadoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_producto_estado=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_producto_estado=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("estado_producto")
...
def productoestado_vista_pantalla(request):
...
def productoestado_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productoestado_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoEstadoInformeView()
...
def productoestado_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
