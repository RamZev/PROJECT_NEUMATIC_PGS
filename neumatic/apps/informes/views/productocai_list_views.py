...
class ProductoCaiInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_cai=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_cai=False)
				case "todos":
...
def productocai_vista_pantalla(request):
...
def productocai_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productocai_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoCaiInformeView()
...
def productocai_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
