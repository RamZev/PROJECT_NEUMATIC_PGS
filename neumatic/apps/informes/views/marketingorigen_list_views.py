# neumatic\apps\informes\views\marketingorigen_list_views.py
...
from ..forms.buscador_marketin_origen_forms import BuscadorMarketingOrigenForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Marketing Origen"
	
	#-- Modelo.
	model = MarketingOrigen
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorMarketingOrigenForm
...
class MarketingOrigenInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_marketing_origen=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_marketing_origen=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_marketing_origen")
...
def marketingorigen_vista_pantalla(request):
...
def marketingorigen_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def marketingorigen_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = MarketingOrigenInformeView()
...
def marketingorigen_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
