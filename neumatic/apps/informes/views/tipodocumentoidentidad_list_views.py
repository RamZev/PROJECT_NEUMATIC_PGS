# neumatic\apps\informes\views\tipodocumentoidentidad_list_views.py
...
from ..forms.buscador_tipodocumentoidentidad_forms import BuscadorTipoDocumentoIdentidadForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Tipos de Documentos de Identidad"
	
	#-- Modelo.
	model = TipoDocumentoIdentidad
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorTipoDocumentoIdentidadForm
...
class TipoDocumentoIdentidadInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_documento_identidad=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_documento_identidad=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("tipo_documento_identidad")
...
def tipodocumentoidentidad_vista_pantalla(request):
...
def tipodocumentoidentidad_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def tipodocumentoidentidad_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = TipoDocumentoIdentidadInformeView()
...
def tipodocumentoidentidad_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
