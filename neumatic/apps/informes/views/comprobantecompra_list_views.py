# neumatic\apps\informes\views\comprobantecompra_list_views.py
...
from ..forms.buscador_comprobantecompra_forms import BuscadorComprobanteCompraForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Comprobantes de Compra"
	
	#-- Modelo.
	model = ComprobanteCompra
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorComprobanteCompraForm
...
class ComprobanteCompraInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_comprobante_compra=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_comprobante_compra=False)
				case "todos":
...
def comprobantecompra_vista_pantalla(request):
...
def comprobantecompra_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def comprobantecompra_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ComprobanteCompraInformeView()
...
def comprobantecompra_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
