# neumatic\apps\informes\views\tarjeta_list_views.py
...
from ..forms.buscador_tarjeta_forms import BuscadorTarjetaForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Tarjetas"
	
	#-- Modelo.
	model = Tarjeta
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorTarjetaForm
...
class TarjetaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_tarjeta=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_tarjeta=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_tarjeta")
...
def tarjeta_vista_pantalla(request):
...
def tarjeta_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = TarjetaInformeView()
