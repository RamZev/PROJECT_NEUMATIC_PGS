# neumatic\apps\informes\views\tiporetencionib_list_views.py
...
from apps.maestros.models.base_models import TipoRetencionIb
from ..forms.buscador_tiporetencionib_forms import BuscadorTipoRetecionIbForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Tipos de Retención IB"
	
	#-- Modelo.
	model = TipoRetencionIb
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorTipoRetecionIbForm
...
class TipoRetencionIbInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_retencion_ib=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_retencion_ib=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("descripcion_tipo_retencion_ib")
...
def tiporetencionib_vista_pantalla(request):
...
def tiporetencionib_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def tiporetencionib_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = TipoRetencionIbInformeView()
...
def tiporetencionib_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
