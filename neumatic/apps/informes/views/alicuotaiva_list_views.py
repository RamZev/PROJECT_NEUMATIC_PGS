# neumatic\apps\informes\views\alicuotaiva_list_views.py
...
from ..forms.buscador_alicuotaiva_forms import BuscadorAlicuotaIvaForm
...
class ConfigViews:
	...
	form_class = BuscadorAlicuotaIvaForm
...
class AlicuotaIvaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_alicuota_iva=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_alicuota_iva=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("alicuota_iva")
...
def alicuotaiva_vista_pantalla(request):
...
def alicuotaiva_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def alicuotaiva_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = AlicuotaIvaInformeView()
...
def alicuotaiva_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
