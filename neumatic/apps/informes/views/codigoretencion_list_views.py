# neumatic\apps\informes\views\codigoretencion_list_views.py
...
from ..forms.buscador_codigoretencion_forms import BuscadorCodigoRetencionForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Códigos de Retención"
	
	#-- Modelo.
	model = CodigoRetencion
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorCodigoRetencionForm
	...
	table_info = {
		...
		"nombre_codigo_retencion": {
			"label": "Nombre Código de Retención",
			"col_width_pdf": 180,
			"pdf_paragraph": False,
			"date_format": None,
			"pdf": True,
...
class CodigoRetencionInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_cod_retencion=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_cod_retencion=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_codigo_retencion")
...
def codigoretencion_vista_pantalla(request):
...
def codigoretencion_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def codigoretencion_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = CodigoRetencionInformeView()
...
def codigoretencion_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
