# neumatic\apps\informes\views\actividad_list_views.py
...
from ..forms.buscador_actividad_forms import BuscadorActividadForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Actividades"
	
	#-- Modelo.
	model = Actividad
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorActividadForm
...
class ActividadInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_actividad=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_actividad=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("descripcion_actividad")
...
def actividad_vista_pantalla(request):
...
def actividad_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def actividad_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ActividadInformeView()
...
def actividad_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
