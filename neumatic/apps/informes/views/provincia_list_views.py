...
class ProvinciaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_provincia=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_provincia=False)
				case "todos":
...
def provincia_vista_pantalla(request):
...
def provincia_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def provincia_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProvinciaInformeView()
...
def provincia_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
