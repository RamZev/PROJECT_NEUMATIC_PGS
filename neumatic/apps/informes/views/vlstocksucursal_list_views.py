# neumatic\apps\informes\views\vlstocksucursal_list_views.py
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Listado de Stock por Sucursal"
	
	#-- Modelo.
	model = VLStockSucursal
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorStockSucursalForm
...
class VLStockSucursalInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		id_deposito = cleaned_data.get('deposito')
		...
		queryset = VLStockSucursal.objects.obtener_datos(
			id_deposito.id_producto_deposito,
			id_familia_desde,
			id_familia_hasta,
			id_marca_desde,
			id_marca_hasta,
	...
	def obtener_contexto_reporte(self, queryset, cleaned_data):
		...
		id_modelo_desde = cleaned_data.get('id_modelo_desde', None)
		id_modelo_hasta = cleaned_data.get('id_modelo_hasta', None)
		valorizado = cleaned_data.get('valorizado', False)
		...
		param_left = {
			"Depósito": id_deposito.nombre_producto_deposito,
			"Inventario Valorizado": "Sí" if valorizado else "No",
		}
		param_right = {
			"Familia": familia,
			"Marca": marca,
		...
		for obj in queryset_list:
			...
			grouped_data[id_familia]['stf_cantidad'] += obj['stock']
			grouped_data[id_familia]['stf_importe'] += float((obj['costo_inventario'] or 0))
			
			#-- Acumular totales por Modelo.
			grouped_data[id_familia]['modelos'][id_modelo]['stm_cantidad'] += obj['stock']
			grouped_data[id_familia]['modelos'][id_modelo]['stm_importe'] += float((obj['costo_inventario'] or 0))
			
			#-- Acumular totales generales.
			tg_cantidad += obj['stock']
			tg_importe += float((obj['costo_inventario'] or 0))
...
def vlstocksucursal_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def generar_pdf(contexto_reporte):
	...
	for familia_data in contexto_reporte.get("objetos", {}).values():
		...
		for modelo_data in familia_data["modelos"].values():
			...
			for obj in modelo_data['detalle']:
				...
				table_data.append([
					...
					formato_argentino(obj['stock']),
					formato_argentino(obj['costo_inventario']) if valorizado else ""
				])
			...
			table_data.append(
				...
				[
					...
					formato_argentino(modelo_data["stm_importe"]) if valorizado else ""
				]
			)
		...
		table_data.append(
			...
			[
				...
				formato_argentino(familia_data["stf_importe"]) if valorizado else ""
			]
		)
		#-- Aplicar estilos a la fila de total (fila actual).
		table_style_config.extend([
	...
	tg_importe = contexto_reporte.get("tg_importe", 0) 
	...
	table_data.append(
		...
		[
			...
			formato_argentino(tg_importe) if valorizado else ""
		]
	)
...
def vlstocksucursal_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = VLStockSucursalInformeView()
...
def vlstocksucursal_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
