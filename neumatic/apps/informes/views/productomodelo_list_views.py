# neumatic\apps\informes\views\productomodelo_list_views.py
...
from ..forms.buscador_productomodelo_forms import BuscadorProductoModeloForm
...
class ConfigViews:
	...
	form_class = BuscadorProductoModeloForm
...
class ProductoModeloInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_modelo=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_modelo=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_modelo")
...
def productomodelo_vista_pantalla(request):
...
def productomodelo_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productomodelo_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoModeloInformeView()
...
def productomodelo_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
