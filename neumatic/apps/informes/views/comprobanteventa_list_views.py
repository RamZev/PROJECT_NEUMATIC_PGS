...
class ComprobanteVentaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_comprobante_venta=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_comprobante_venta=False)
				case "todos":
...
def comprobanteventa_vista_pantalla(request):
...
def comprobanteventa_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def comprobanteventa_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ComprobanteVentaInformeView()
...
def comprobanteventa_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
