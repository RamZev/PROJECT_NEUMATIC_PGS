# neumatic\apps\informes\views\conceptobanco_list_views.py
...
from ..forms.buscador_conceptobanco_forms import BuscadorConceptoBancoForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Conceptos de Banco"
	
	#-- Modelo.
	model = ConceptoBanco
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorConceptoBancoForm
...
class ConceptoBancoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_concepto_banco=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_concepto_banco=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_concepto_banco")
...
def conceptobanco_vista_pantalla(request):
...
def conceptobanco_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def conceptobanco_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ConceptoBancoInformeView()
...
def conceptobanco_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
