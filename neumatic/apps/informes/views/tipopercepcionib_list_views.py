# neumatic\apps\informes\views\tipopercepcionib_list_views.py
...
class TipoPercepcionIbInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_percepcion_ib=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_percepcion_ib=False)
				case "todos":
...
def tipopercepcionib_vista_pantalla(request):
...
def tipopercepcionib_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def tipopercepcionib_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = TipoPercepcionIbInformeView()
...
def tipopercepcionib_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
