...
class CuentaBancoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		for obj in queryset:
			...
			obj_dict['nombre_banco'] = obj.id_banco.nombre_banco if obj.id_banco else ""
			obj_dict['nombre_moneda'] = obj.id_moneda.nombre_moneda if obj.id_moneda else ""
			obj_dict['tipo_cuenta_display'] = obj.tipo_cuenta_display
...
def cuentabanco_vista_pantalla(request):
...
def cuentabanco_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def cuentabanco_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = CuentaBancoInformeView()
...
def cuentabanco_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
