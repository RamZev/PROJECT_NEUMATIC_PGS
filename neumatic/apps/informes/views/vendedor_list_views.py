...
class VendedorInformeView(InformeFormView):
...
def vendedor_vista_pantalla(request):
...
def vendedor_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def vendedor_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = VendedorInformeView()
...
def vendedor_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
