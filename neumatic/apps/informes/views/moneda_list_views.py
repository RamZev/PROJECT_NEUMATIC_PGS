# neumatic\apps\informes\views\moneda_list_views.py
...
from ..forms.buscador_moneda_forms import BuscadorMonedaForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Monedas"
	
	#-- Modelo.
	model = Moneda
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorMonedaForm
...
class MonedaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_moneda=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_moneda=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_moneda")
...
def moneda_vista_pantalla(request):
...
def moneda_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = MonedaInformeView()
