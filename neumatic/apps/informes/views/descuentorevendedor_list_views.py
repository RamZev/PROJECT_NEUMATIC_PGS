# neumatic\apps\informes\views\descuentorevendedor_list_views.py
...
from apps.maestros.models.descuento_vendedor_models import DescuentoRevendedor
from ..forms.buscador_descuentorevendedor_forms import BuscadorDescuentoRevendedorForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Descuento Revendedor"
	
	#-- Modelo.
	model = DescuentoRevendedor
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorDescuentoRevendedorForm
...
class DescuentoRevendedorInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		#-- Aplicar filtros.
		if estatus == "activos":
			queryset = queryset.filter(estatus_descuento_revendedor=True)
		elif estatus == "inactivos":
			queryset = queryset.filter(estatus_descuento_revendedor=False)
...
def descuentorevendedor_vista_pantalla(request):
...
def descuentorevendedor_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def descuentorevendedor_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = DescuentoRevendedorInformeView()
...
def descuentorevendedor_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
