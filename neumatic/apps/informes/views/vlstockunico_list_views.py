# apps\informes\views\vlstockunico_list_views.py
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Listado de Stock Único"
	
	#-- Modelo.
	model = VLStockUnico
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorStockUnicoForm
...
class VLStockUnicoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		queryset = VLStockUnico.objects.obtener_datos()
		return queryset
	...
	def obtener_contexto_reporte(self, queryset, cleaned_data):
		...
		for obj in queryset_list:
			...
			grouped_data[id_familia]['stf_stock'] += obj['stock']
			
			#-- Acumular totales por Modelo.
			grouped_data[id_familia]['modelos'][id_modelo]['stm_stock'] += obj['stock']
			
			#-- Acumular totales generales.
			stock_general += obj['stock']
...
def vlstockunico_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def generar_pdf(contexto_reporte):
	...
	#-- Fila Total General.
	stock_general = contexto_reporte.get("stock_general", 0)
	
	table_data.append(
		[""]*5 + 
		[
			"Total Genral:", 
			formato_argentino_entero(stock_general),
		]
	)
...
def vlstockunico_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = VLStockUnicoInformeView()
...
def vlstockunico_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
