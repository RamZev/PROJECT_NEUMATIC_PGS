...
class OperarioInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_operario=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_operario=False)
				case "todos":
...
def operario_vista_pantalla(request):
...
def operario_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def operario_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = OperarioInformeView()
...
def operario_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
