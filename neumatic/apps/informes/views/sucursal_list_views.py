...
class SucursalInformeView(InformeFormView):
...
def sucursal_vista_pantalla(request):
...
def sucursal_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def sucursal_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = SucursalInformeView()
...
def sucursal_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
