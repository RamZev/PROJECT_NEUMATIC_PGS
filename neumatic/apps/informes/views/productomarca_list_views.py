# neumatic\apps\informes\views\productomarca_list_views.py
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Producto Marca"
	
	#-- Modelo.
	model = ProductoMarca
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorProductoMarcaForm
...
class ProductoMarcaInformeView(InformeFormView):
...
def productomarca_vista_pantalla(request):
...
def productomarca_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productomarca_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoMarcaInformeView()
...
def productomarca_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
