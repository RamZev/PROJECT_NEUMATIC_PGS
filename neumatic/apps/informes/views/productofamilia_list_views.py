...
from ..forms.buscador_productofamilia_forms import BuscadorProductoFamiliaForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Producto Familia"
	
	#-- Modelo.
	model = ProductoFamilia
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorProductoFamiliaForm
...
class ProductoFamiliaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_producto_familia=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_producto_familia=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_producto_familia")
...
def productofamilia_vista_pantalla(request):
...
def productofamilia_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productofamilia_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoFamiliaInformeView()
...
def productofamilia_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
