# neumatic\apps\informes\views\vlpercepibsubcuentadetallado_list_views.py
...
class ConfigViews:
	...
	form_class = BuscadorPercepIBSubcuentaDetalladoForm
...
class VLPercepIBSubcuentaDetalladoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		return VLPercepIBSubcuentaDetallado.objects.obtener_datos(fecha_desde, fecha_hasta)
	...
	def obtener_contexto_reporte(self, queryset, cleaned_data):
		...
		for obj in queryset:
			sub_cuenta = obj.sub_cuenta  # Campo que agrupa los datos.
			if sub_cuenta not in grouped_data:
				grouped_data[sub_cuenta] = {
					'nombre_subcuenta': obj.nombre_cliente_padre,
					'comprobantes': [],
					'total_neto': Decimal(0),
					'total_percep': Decimal(0),
				}
			#-- Añadir el comprobante al grupo de comprobantes.
			grouped_data[sub_cuenta]['comprobantes'].append(obj)
			#-- Calcular el subtotal por Sub cuenta.
			grouped_data[sub_cuenta]['total_neto'] += obj.neto
			grouped_data[sub_cuenta]['total_percep'] += obj.percep_ib
		
		#-- Convertir los datos agrupados a un formato serializable:
		#-- Se recorre cada grupo y se convierte cada comprobante a diccionario usando raw_to_dict.
...
def vlpercepibsubcuentadetallado_vista_excel(request):
	...
	view_instance = VLPercepIBSubcuentaDetalladoInformeView()
...
def vlpercepibsubcuentadetallado_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
