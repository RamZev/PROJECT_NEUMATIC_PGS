# neumatic\apps\informes\views\productodeposito_list_views.py
...
from ..forms.buscador_productodeposito_forms import BuscadorProductoDepositoForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Producto Depósito"
	
	#-- Modelo.
	model = ProductoDeposito
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorProductoDepositoForm
...
class ProductoDepositoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		#-- Aplicar filtros.
		if estatus == "activos":
			queryset = queryset.filter(estatus_producto_deposito=True)
		elif estatus == "inactivos":
			queryset = queryset.filter(estatus_producto_deposito=False)
		
		if sucursal:
			queryset = queryset.filter(id_sucursal=sucursal)
		
		#-- Ordenar los resultados.
		queryset = queryset.order_by('id_sucursal__nombre_sucursal', 'nombre_producto_deposito')
...
def productodeposito_vista_pantalla(request):
...
def productodeposito_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def productodeposito_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = ProductoDepositoInformeView()
...
def productodeposito_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
