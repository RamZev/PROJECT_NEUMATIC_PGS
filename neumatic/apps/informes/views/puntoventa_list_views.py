# neumatic\apps\informes\views\puntoventa_list_views.py
...
class PuntoVentaInformeView(InformeFormView):
...
def puntoventa_vista_pantalla(request):
...
def puntoventa_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def puntoventa_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = PuntoVentaInformeView()
...
def puntoventa_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
