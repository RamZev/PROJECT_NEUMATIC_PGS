# neumatic\apps\informes\views\tipoiva_list_views.py
...
from ..forms.buscador_tipoiva_forms import BuscadorTipoIvaForm
...
class ConfigViews:
	...
	#-- Título del reporte.
	report_title = "Reporte de Tipos de IVA"
	
	#-- Modelo.
	model = TipoIva
	
	#-- Formulario asociado al modelo.
	form_class = BuscadorTipoIvaForm
...
class TipoIvaInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_iva=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_tipo_iva=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by("nombre_iva")
...
def tipoiva_vista_pantalla(request):
...
def tipoiva_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = TipoIvaInformeView()
