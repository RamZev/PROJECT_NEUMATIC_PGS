...
from ..forms.buscador_mediopago_forms import BuscadorMedioPagoForm
...
class ConfigViews:
	...
	form_class = BuscadorMedioPagoForm
...
class MedioPagoInformeView(InformeFormView):
	...
	def obtener_queryset(self, cleaned_data):
		...
		if estatus:
			match estatus:
				case "activos":
					queryset = ConfigViews.model.objects.filter(estatus_medio_pago=True)
				case "inactivos":
					queryset = ConfigViews.model.objects.filter(estatus_medio_pago=False)
				case "todos":
					queryset = ConfigViews.model.objects.all()
		
		queryset = queryset.order_by('nombre_medio_pago')
		...
		for obj in queryset:
			...
			obj_dict['condicion_display'] = obj.condicion_medio_pago_display
...
def mediopago_vista_pantalla(request):
...
def mediopago_vista_pdf(request):
	#-- Obtener el token de la querystring.
	token = request.GET.get("token")
...
def mediopago_vista_excel(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
	...
	view_instance = MedioPagoInformeView()
...
def mediopago_vista_csv(request):
	token = request.GET.get("token")
	if not token:
		return HttpResponse("Token no proporcionado", status=400)
